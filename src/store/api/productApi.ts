...
export interface ProductDTO {
  ...
  defaultFabricInventoryIds?: number[];
}
...
export const productApi = baseSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // Get available additional fabrics (inventory not attached as defaults)
    getAvailableAdditionalFabrics: builder.query<ProductFabric[], number>({
      query: (productId) => `/product/${productId}/available-additional-fabrics`,
      providesTags: (result, error, productId) => [
        { type: "Products", id: `available-fabrics-${productId}` },
      ],
    }),
...
export const {
  ...
  useGetAvailableAdditionalFabricsQuery,
